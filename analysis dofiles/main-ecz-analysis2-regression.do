/*******************************************************************************
DO FILE NAME:			main-ecz-analysis2-regression.do
...
global filename "main-ecz-analysis-regression"
...
/*----------------------------------------------------------------------------*/
* MODEL 3 (model 1 further adjusted for mediating effect of sleep problems)
*recode those with missing sleep as not having sleep problems
recode sleep (.=0)

*run analysis
stcox i.exposed i.calendarperiod i.practice_carstairs i.sleep, strata(setid) level(95) base
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
...
*run analysis
stcox i.exposed i.calendarperiod i.practice_carstairs i.steroids, strata(setid) level(95) base
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
...
*******************************************************************************/
* create excel file
putexcel set "${pathResults}/main-ecz-analysis.xlsx", sheet(regression_main) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Association (HR [95% CI]) between atopic eczema and severe mental illness: comparing risk of SMI in those with atopic eczema to those without. ", bold
...
} /*end foreach model in minimally model1 model2 model3 model4 */
local ++rowcount
