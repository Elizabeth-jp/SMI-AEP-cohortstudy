...
TASK:					Repeat the main analysis looking for effect modification 
						...
						Model 1 (model adjusted for confounders - deprivation
						and calendar period)
						
DATASET(S)/FILES USED:	cohort-psoriasis-main.dta
						

DATASETS CREATED:		main-pso-analysis-interaction.xls
*******************************************************************************/
/*******************************************************************************
HOUSEKEEPING
*******************************************************************************/
...
global filename "main-pso-analysis-interaction"
...
/* 
Will be using model 1 adjusted for confounders - deprivation and carstairs
there are individuals with missing cartstairs that need to be excluded
...
*run analysis in men 
display in red "**************** men ********************************"
stcox i.exposed i.calendarperiod i.practice_carstairs if sex==1, strata(setid) level(95) base
matrix table = r(table) 
gethr table sexM

*run analysis in women
display in red "**************** women ********************************"
stcox i.exposed i.calendarperiod i.practice_carstairs if sex==2, strata(setid) level(95) base
matrix table = r(table) 
gethr table sexF

*run interaction analysis 
display in red "**************** sex interaction ********************************"
stcox i.exposed##i.sex i.calendarperiod i.practice_carstairs, strata(setid) level(95) base
matrix table = r(table) 
...
foreach age in 1 2 3 4 {
	...
	*run analysis for age group
	display in red "************************ age_grp `age' ****************************"
	stcox i.exposed i.calendarperiod i.practice_carstairs if age_grp==`age', strata(setid) level(95) base
	matrix table = r(table)
	gethr table age`age'
}
...
est store A
*interaction model
stcox i.exposed##i.age_grp i.calendarperiod i.practice_carstairs, strata(setid) level(95) base
est store B
lrtest A B
...
foreach period in 1997 2004 2010 2016 {
	...
	*run analysis for calendar period
	display in red "************************ calendarperiod `period' ****************************"
	stcox i.exposed i.practice_carstairs if calendarperiod==`period', strata(setid) level(95) base
	matrix table = r(table)
...
* create excel file
putexcel set "${pathResults}/main-pso-analysis.xlsx", sheet(interaction_sex) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Adjusted hazard ratios (95% CIs) for the association between psoriasis and severe mental illness, stratified by sex (adjusted for calendar period and Carstairs deprivation).", bold
local ++rowcount 
...
* create excel file
putexcel set "${pathResults}/main-pso-analysis.xlsx", sheet(interaction_age) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Adjusted hazard ratios (95% CIs) for the association between psoriasis and severe mental illness, stratified by current age (adjusted for calendar period and Carstairs deprivation).", bold
local ++rowcount 
putexcel A`rowcount'="Fitted to patients with complete data for all variables included in each model and from valid matched sets*"
local ++rowcount
...
* create excel file
putexcel set "${pathResults}/main-pso-analysis.xlsx", sheet(interaction_calendarperiod) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Adjusted hazard ratios (95% CIs) for the association between psoriasis and severe mental illness, stratified by calendar period (adjusted for Carstairs deprivation).", bold
local ++rowcount 
putexcel A`rowcount'="Fitted to patients with complete data for all variables included in each model and from valid matched sets*"
local ++rowcount
