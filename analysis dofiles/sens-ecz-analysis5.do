/*******************************************************************************
DO FILE NAME:			sens-ecz-analysis5.do
...
global filename "sens-ecz-analysis5"
...
forvalues x=1/13 {
	use ${pathData}/Consultation_extract_ecz_extract3_`x', clear
	gen contact=0
	...
	*save file
	if `x'==1 save ${pathOut}/ecz_consultation_dates
	if `x'!=1 {
		append using ${pathOut}/ecz_consultation_dates
		save ${pathOut}/ecz_consultation_dates, replace
	}		
}
...
compress
label data "consultation dates for all eczema patients"
notes: consultation dates for all eczema patients
...
*******************************************************************************/
use ${pathDatain}/ecz_indexdates_1record, clear
drop indexdate2
rename indexdate1 indexdate 

*merge in consultation data
merge 1:m patid using ${pathOut}/ecz_consultation_dates, keep(match master) nogen
...
*******************************************************************************/
* create excel file
putexcel set "${pathResults}/appendix-ecz-analysis.xlsx", sheet(sens5) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Sensitivity analysis 5: Association (HR [95% CI]) between atopic eczema and severe mental illness: comparing risk of SMI in those with atopic eczema to those without (consultation in yr before indexdate) ", bold
