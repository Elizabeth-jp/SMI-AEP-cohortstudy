/*******************************************************************************
DO FILE NAME:			appendix-pso-missingCharacteristics.do
...
TASK:					Create a table for the appendix including data for:
						1. the whole cohort (with psoriasis and without psoriasis)
...
global filename "apdx-pso-missingCharacteristics"
...
#1. Identify the different analysis samples and open the dataset
	all - whole cohort (with psoriasis and without psoriasis)
	model1 - cohort with complete cases with no missing carstairs deprivation
	noCar - missing carstairs
	model3 - cohort with complete cases with no missing BMI or smoking 
	noBMI - missing BMI
	noSmok - missing smoking status
*******************************************************************************/
foreach sample in all model1 noCar model3 noBMI noSmok {
	...
	***************************************************************************/
	/*--------------------------------------------------------------------------
		...
		----------------------------------------------------------------------*/
		local bincv " "harmfulalcohol" "sex" "
		foreach group in exp unexp {
		    foreach cv in `bincv' {
...
putexcel D`rowcount'="Sample included in model adjusting for potential confounders (Model 1)"
putexcel F`rowcount'="Individuals with missing Carstairs data"
putexcel H`rowcount'="Sample included in model adjusting for potential mediators (Model 3)"
putexcel J`rowcount'="Individuals with missing BMI status"
putexcel L`rowcount'="Individuals with missing smoking status"
putexcel A`rowcount':M`rowcount', overwritefmt bold border(top, thin, black) 
local ++rowcount
...
foreach pop in all model1 noCar model3 noBMI noSmok {
	forvalues i=1/2 {
		local col`i' : word `i' of ``pop'' // col1==exp; col2==unexp
	} /*end forvalues i=1/2*/ 
	putexcel `col1'`rowcount'="With psoriasis"
	putexcel `col2'`rowcount'="Without psoriasis"
}/*end foreach pop in all model1 noCar model3 noBMI noSmok*/
...
forvalues x=1/6 {
	...
	foreach pop in all model1 noCar model3 noBMI noSmok {
		...
		*deal with missing information
		...
		if "`pop'"=="model3" & `x'==6 { // for missing carstairs in model3
			putexcel `col1'`rowcount'="n/a"
			putexcel `col2'`rowcount'="n/a"
		} /*end if "`pop'"=="model3" & `x'==6 */
	...
	* put output strings in excel file
	foreach pop in all model1 noCar model3 noBMI noSmok {
		...
		* deal with missing information
		...
		if "`pop'"=="model3" & `j'==4 { // for missing status in model3
			putexcel `col1'`rowcount'="n/a"
			putexcel `col2'`rowcount'="n/a"
		} /*end if "`pop'"=="model3" & `j'==4 */
	}/*end foreach pop in all model1 noCar model3 noBMI noSmok*/	
	...
	* put output strings in excel file
	foreach pop in all model1 noCar model3 noBMI noSmok {
		...
		* deal with missing information
		...
		if "`pop'"=="model3" & `x'==3 { // for missing status in model3
			putexcel `col1'`rowcount'="n/a"
			putexcel `col2'`rowcount'="n/a"
		} /*end if "`pop'"=="model3" & `x'==3 */
	}/*end foreach pop in all model1 noCar model3 noBMI noSmok*/	
...
foreach j of local levels {
	putexcel A`rowcount'="`: label (ethnicity) `j''" // use variable label for row caption
	
	foreach pop in all model1 noCar model3 noBMI noSmok {
		forvalues i=1/2 {
			local col`i' : word `i' of ``pop'' 
		} /*end forvalues i=1/2*/ 
		putexcel `col1'`rowcount'="${`pop'_ethnicity`j'_exp}"
		putexcel `col2'`rowcount'="${`pop'_ethnicity`j'_unexp}"
	}/*end foreach pop in all model1 noCar model3 noBMI noSmok*/	
...
foreach j of local levels {
	putexcel A`rowcount'="`: label (cci) `j''" // use variable label for row caption
	
	foreach pop in all model1 noCar model3 noBMI noSmok {
		forvalues i=1/2 {
			local col`i' : word `i' of ``pop'' 
		} /*end forvalues i=1/2*/ 
		putexcel `col1'`rowcount'="${`pop'_cci`j'_exp}"
		putexcel `col2'`rowcount'="${`pop'_cci`j'_unexp}"
	}/*end foreach pop in all model1 noCar model3 noBMI noSmok*/	
...
* center cell content
putexcel B5:M`rowcount', overwritefmt hcenter
local ++rowcount
local ++rowcount
...
putexcel A`rowcount'="Individuals can contribute data as both psoriasis exposed and unexposed. Therefore, numbers of exposed/unexposed do not total the whole cohort, as individuals may be included in more than one column."
local ++rowcount
