/*******************************************************************************
DO FILE NAME:			main-pso-analysis3-interaction.do
...
TASK:					Repeat the main analysis looking for effect modification 
						...
						Model  (model adjusted for confounders and mediators)
						
DATASET(S)/FILES USED:	cohort-psoriasis-main.dta
						

DATASETS CREATED:		main-pso-analysis-interaction-mediators.xls
*******************************************************************************/
/*******************************************************************************
HOUSEKEEPING
*******************************************************************************/
...
global filename "main-pso-analysis-interaction-mediators"
...
/* 
Will be using model 6 adjusted for confounders and mediators
there are individuals with missing cartstairs, bmi and smoking that need to be excluded
...
*run analysis in men 
display in red "**************** men ********************************"
stcox i.exposed i.calendarperiod i.practice_carstairs i.cci i.harmfulalcohol i.smokstatus i.bmi_cat if sex==1, strata(setid) level(95) base
matrix table = r(table) 
gethr table sexM

*run analysis in women
display in red "**************** women ********************************"
stcox i.exposed i.calendarperiod i.practice_carstairs i.cci i.harmfulalcohol i.smokstatus i.bmi_cat if sex==2, strata(setid) level(95) base
matrix table = r(table) 
gethr table sexF

*run interaction analysis 
display in red "**************** sex interaction ********************************"
stcox i.exposed##i.sex i.calendarperiod i.practice_carstairs i.cci i.harmfulalcohol i.smokstatus i.bmi_cat, strata(setid) level(95) base
matrix table = r(table) 
...
foreach age in 1 2 3 4 {
	...
	*run analysis for age group
	display in red "************************ age_grp `age' ****************************"
	stcox i.exposed i.calendarperiod i.practice_carstairs i.cci i.harmfulalcohol i.smokstatus i.bmi_cat if age_grp==`age', strata(setid) level(95) base
	matrix table = r(table)
	gethr table age`age'
}
...
est store A
*interaction model
stcox i.exposed##i.age_grp i.calendarperiod i.practice_carstairs i.cci i.harmfulalcohol i.smokstatus i.bmi_cat, strata(setid) level(95) base
est store B
lrtest A B
...
foreach period in 1997 2004 2010 2016 {
	...
	*run analysis for calendar period
	display in red "************************ calendarperiod `period' ****************************"
	stcox i.exposed i.practice_carstairs i.cci i.harmfulalcohol i.smokstatus i.bmi_cat if calendarperiod==`period', strata(setid) level(95) base
	matrix table = r(table)
...
* create excel file
putexcel set "${pathResults}/main-pso-analysis.xlsx", sheet(interaction_sex_mediators) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Adjusted hazard ratios (95% CIs) for the association between psoriasis and severe mental illness, stratified by sex (adjusted for confounders and mediators).", bold
local ++rowcount 
...
* exposed and unexposed
	foreach exp in N Y {
	if "`exp'"=="N" putexcel A`rowcount'="No psoriasis"
	if "`exp'"=="Y" putexcel A`rowcount'="psoriasis"			
...
* create excel file
putexcel set "${pathResults}/main-pso-analysis.xlsx", sheet(interaction_age_mediators) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Adjusted hazard ratios (95% CIs) for the association between psoriasis and severe mental illness, stratified by current age (adjusted for confounders and mediators).", bold
local ++rowcount 
putexcel A`rowcount'="Fitted to patients with complete data for all variables included in each model and from valid matched sets*"
local ++rowcount
...
* create excel file
putexcel set "${pathResults}/main-pso-analysis.xlsx", sheet(interaction_cd_mediators) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Adjusted hazard ratios (95% CIs) for the association between psoriasis and severe mental illness, stratified by calendar period (adjusted for confounders and mediators).", bold
local ++rowcount 
putexcel A`rowcount'="Fitted to patients with complete data for all variables included in each model and from valid matched sets*"
local ++rowcount
