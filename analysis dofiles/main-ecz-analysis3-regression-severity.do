/*******************************************************************************
DO FILE NAME:			main-ecz-analysis3-regression-severity.do
...
global filename "main-ecz-analysis-regression-severity"
...
*******************************************************************************/
*create program to get the results 
...
program define results
	...
	* pull out HRs (95% CI)
	local x=2
	foreach severity in mild moderate severe {
	...
	* pull out n's
	foreach severity in unexposed mild moderate severe {
		if "`severity'"=="unexposed" stdescribe if severity==0
		if "`severity'"=="mild" stdescribe if severity==1
		if "`severity'"=="moderate" stdescribe if severity==2
		if "`severity'"=="severe" stdescribe if severity==3
...
label define severity 0"Unexposed" 1"Exposed – mild" 2"Exposed – moderate" 3 "Exposed – severe"
...
/*----------------------------------------------------------------------------*/
* MODEL 3(model 1 further adjusted for mediating effect of problems with sleep)
recode sleep (.=0)

*run analysis
stcox i.severity i.calendarperiod i.practice_carstairs i.sleep, strata(setid) level(95) base
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
...
/*----------------------------------------------------------------------------*/
* MODEL 4 (model 1 further adjusted for mediating effect of harmful alcohol use)
...
/*----------------------------------------------------------------------------*/
* MODEL 5 (model 1 further adjusted for mediating effect of high dose steroids)
*recode those with missing steroids as not having them
recode steroids (.=0)

*run analysis
stcox i.severity i.calendarperiod i.practice_carstairs i.steroids, strata(setid) level(95) base
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
...
/*----------------------------------------------------------------------------*/
* MODEL 6 (model 1 further adjusted for mediating effect of smoking)
...
/*----------------------------------------------------------------------------*/
* MODEL 7 (model 1 further adjusted for mediating effect of BMI)
...
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
results table model7
*calls the results program and gives the matrix name and name of the model

/*----------------------------------------------------------------------------*/
* MODEL 8 (model 1 adjusted for all mediators)
...
*run analysis 
stcox i.severity i.calendarperiod i.practice_carstairs i.cci i.sleep i.harmfulalcohol i.smokstatus i.bmi_cat i.steroids, strata(setid) level(95) base
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
results table model8
*calls the results program and gives the matrix name and name of the model
...
putexcel set "${pathResults}/main-ecz-analysis.xlsx", sheet(regression_severity) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Association (HR [95% CI]) between severity of atopic eczema and severe mental illness. ", bold
local ++rowcount 
...
putexcel N`rowcount'="Model 3 (Model 1 further adjusted for problems with sleep)", bold border(top, thin, black)
putexcel R`rowcount'="Model 4 (Model 1 further adjusted for harmful alcohol use)", bold border(top, thin, black)
putexcel V`rowcount'="Model 5 (Model 1 further adjusted for high dose steroids)", bold border(top, thin, black)
putexcel Z`rowcount'="Model 6 (Model 1 further adjusted for smoking status)", bold border(top, thin, black)
putexcel AD`rowcount'="Model 7 (Model 1 further adjusted for BMI)", bold border(top, thin, black)
putexcel AH`rowcount'="Model 8 (Model 1 further adjusted for all mediators****)", bold border(top, thin, black)
putexcel A`rowcount':AK`rowcount', overwritefmt border(top, thin, black)
local ++rowcount 

* format table header cells
putexcel A3:AK3, overwritefmt bold border(top, thin, black) 
putexcel A4:AK4, overwritefmt bold border(top, thin, black) txtwrap
...
local model7 "AD AE AF AG"
local model8 "AH AI AJ AK"

foreach model in minimally model1 model2 model3 model4 model5 model6 model7 model8 {
	forvalues i=1/4 {
...
} /*end foreach model in minimally model1 model2 model3 model4 model5 model6 model7 model8 */
local ++rowcount

*put in data for each model in excel file 
foreach severity in unexposed mild moderate severe {
		if "`severity'"=="unexposed" putexcel A`rowcount'="Unexposed"
		if "`severity'"=="mild" putexcel A`rowcount'="Mild eczema"
		if "`severity'"=="moderate" putexcel A`rowcount'="Moderate eczema"
		if "`severity'"=="severe" putexcel A`rowcount'="Severe eczema"
		...
		local ++rowcount
	} /*end foreach severity in un mi mod sev*/
