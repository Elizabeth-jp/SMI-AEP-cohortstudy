/*******************************************************************************
DO FILE NAME:			sens-ecz-analysis2.do
...
TASK:					Sensitivity analysis 2 - using less strict definitions of
						sleep (sleep_all variable)
...
global filename "sens-ecz-analysis2"
...
append using "${pathIn}/variables-ecz-sleep-all"
...
collapse (firstnm) modsevere harmfulalcohol sleep_all smi steroids, by(patid date)
...
local update " "modsevere" "harmfulalcohol" "sleep_all" "smi" "steroids" "
...
drop sleep_all modsevere harmfulalcohol smi steroids

rename statemodsevere modsevere
rename statesleep_all sleep_all
...
*save
save "${pathCohort}/ecz-time-updated-variables-sens2", replace

/*******************************************************************************
BUILD ECZEMA COHORT FOR SENSITIVITY ANALYSIS
*******************************************************************************/
/*----------------------------------------------------------------------------*/
...
*save the dataset
save "${pathCohort}/cohort-eczema-sens2-exposed", replace

/*----------------------------------------------------------------------------*/
* Deal with time updated variables

*merge in time-updated variables dataset
merge 1:m patid using "${pathCohort}/ecz-time-updated-variables-sens2"
...
merge m:1 patid using "${pathCohort}/cohort-eczema-sens2-exposed"
...
*save the dataset
save "${pathCohort}/cohort-eczema-sens2-unexposed", replace
...
*merge in exposed cohort 
merge m:1 patid using "${pathCohort}/cohort-eczema-sens2-unexposed"
...
append using "${pathCohort}/cohort-eczema-sens2-exposed"
...
*save the dataset
save "${pathCohort}/cohort-eczema-sens2", replace

*delete interim datasets
erase "${pathCohort}/cohort-eczema-sens2-exposed.dta"
erase "${pathCohort}/cohort-eczema-sens2-unexposed.dta"
...
label var sleep_all "Sleep problems 0=No, 1=Yes"
...
label data "stset data for smi outcome - sensitivity analysis 2"
notes: stset data for smi outcome - sensitivity analysis 2

save "${pathCohort}/cohort-eczema-sens2", replace

/*******************************************************************************
REGRESSION ANALYSIS
using the sensitivity analysis 2 dataset, do a minimally adjusted (minimally), confounder 
...
use "${pathCohort}/cohort-eczema-sens2", clear
...
/*----------------------------------------------------------------------------*/
* MODEL 2 (model 1 further adjusted for mediating effect of sleep problems)
*recode those with missing sleep as not having sleep problems
recode sleep_all (.=0)

*run analysis
stcox i.exposed i.calendarperiod i.practice_carstairs i.sleep_all, strata(setid) level(95) base
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
...
/*----------------------------------------------------------------------------*/
* MODEL 3 (model 1 adjusted for all mediators)
...
*run analysis 
stcox i.exposed i.calendarperiod i.practice_carstairs i.cci i.sleep_all i.harmfulalcohol i.smokstatus i.bmi_cat i.steroids, strata(setid) level(95) base
matrix table = r(table) 
*puts results in the matrix so I can pull the numbers out
...
*******************************************************************************/
* create excel file
putexcel set "${pathResults}/appendix-ecz-analysis.xlsx", sheet(sens2) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Sensitivity analysis 2: Association (HR [95% CI]) between atopic eczema and severe mental illness: effects of sleep ", bold
...
putexcel J`rowcount'="Model 2 (Model 1 further adjusted for sleep problems)", bold border(top, thin, black)
putexcel N`rowcount'="Model 3 (Model 1 further adjusted for mediators***)", bold border(top, thin, black)
putexcel A`rowcount':Q`rowcount', overwritefmt border(top, thin, black)
local ++rowcount 

* format table header cells
putexcel A3:Q3, overwritefmt bold border(top, thin, black) 
putexcel A4:Q4, overwritefmt bold border(top, thin, black) txtwrap
...
foreach model in minimally model1 model2 model3 {
	forvalues i=1/4 {
...
} /*end foreach model in minimally model1 model2 model3 */
local ++rowcount
