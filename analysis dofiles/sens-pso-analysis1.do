/*******************************************************************************
DO FILE NAME:			sens-pso-analysis1.do
...
DATASETS CREATED:		cohort-psoriasis-sens1
						appendix-pso-analysis.xls
						(worksheet sens1)
*******************************************************************************/
/*******************************************************************************
HOUSEKEEPING
...
global filename "sens-pso-analysis1"
...
append using "${pathIn}/outcome-pso-smi-all"

sort patid date
order patid date

*collapse on patid and date so there is one record per patient
collapse (firstnm) psoriasis_severity harmfulalcohol smi_all, by(patid date)
...
local update " "psoriasis_severity" "harmfulalcohol" "smi_all" "
...
drop psoriasis_severity harmfulalcohol smi_all 
...
*save
save "${pathCohort}/pso-time-updated-variables-sens1", replace

/*******************************************************************************
#1: BUILD psoriasis COHORT FOR SENSITIVITY ANALYSIS FIRST 
*******************************************************************************/
/*----------------------------------------------------------------------------*/
...
merge 1:1 patid using "${pathIn}/outcome-pso-smi-all.dta"
...
*save the dataset
save "${pathCohort}/cohort-psoriasis-sens1-exposed", replace

/*----------------------------------------------------------------------------*/
* Deal with time updated variables

*merge in time-updated variables dataset
merge 1:m patid using "${pathCohort}/pso-time-updated-variables-sens1"
...
merge m:1 patid using "${pathCohort}/cohort-psoriasis-sens1-exposed"
...
*save the dataset
save "${pathCohort}/cohort-psoriasis-sens1-unexposed", replace
...
*merge in exposed cohort 
merge m:1 patid using "${pathCohort}/cohort-psoriasis-sens1-unexposed"
...
append using "${pathCohort}/cohort-psoriasis-sens1-exposed"
...
*save the dataset
save "${pathCohort}/cohort-psoriasis-sens1", replace

*delete interim datasets
erase "${pathCohort}/cohort-psoriasis-sens1-exposed.dta"
erase "${pathCohort}/cohort-psoriasis-sens1-unexposed.dta"
...
use "${pathCohort}/cohort-psoriasis-sens1", clear
...
*******************************************************************************/
* create excel file
putexcel set "${pathResults}/appendix-pso-analysis.xlsx", sheet(sens1) modify

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Sensitivity analysis 1: Association (HR [95% CI]) between atopic psoriasis and severe mental illness: comparing risk of SMI in those with psoriasis to those without. ", bold
